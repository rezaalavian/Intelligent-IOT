...
def objective(trial: optuna.Trial, max_rows: int) -> float:
    ...
    out_name = f"models/saved_models/hpo_trial_{trial.number}.pt"
    try:
        result = train(
...
def main():
    n_trials = int(os.environ.get("OPT_TRIALS", "4"))
    ...
    params_path = os.path.join(out_dir, f"hpo_best_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}.txt")
